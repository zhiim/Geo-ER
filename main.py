...
parser.add_argument(
    ...
    type=str,
    default="osm_yelp",
    help="Data source (oms_yelp, osm_fsq",
)
parser.add_argument(
    "-c",
    "--city",
    type=str,
    default="pit",
    help="City dataset (sin, edi, tor, pit)",
)
...
train_x, train_coord, train_n, train_y = prepare_dataset(
    train_path, n_train_path, max_seq_len=config.max_seq_len
)


valid_path = config.path + args.source + "/" + args.city + "/valid.txt"
n_valid_path = config.n_path + args.source + "/" + args.city + "/n_valid.json"
valid_x, valid_coord, valid_n, valid_y = prepare_dataset(
    valid_path, n_valid_path, max_seq_len=config.max_seq_len
)


test_path = config.path + args.source + "/" + args.city + "/test.txt"
n_test_path = config.n_path + args.source + "/" + args.city + "/n_test.json"
test_x, test_coord, test_n, test_y = prepare_dataset(
    test_path, n_test_path, max_seq_len=config.max_seq_len
)
...
train_GeoER(
    ...
    test_y,
    device,
    save_path=config.save_path + city.lower(),
